...
module top;
	import "DPI-C" function void svPutBitArrElem1VecValFunction(output bit [7:0] data[], input  int index, input  byte unsigned value);
	bit [7:0] arr[0:7];
	bit [7:0] arr2 [0:7];
	initial 
	begin
		svPutBitArrElem1VecValFunction(arr, 3, 8'hFA);
		svPutBitArrElem1VecValFunction(arr2, 5, 8'hAB);
